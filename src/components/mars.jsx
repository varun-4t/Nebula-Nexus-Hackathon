    ...
    <div className="min-h-screen bg-black overflow-hidden relative">
        ...
        {showSecondVideo && (
          <div className="absolute inset-0 flex items-center justify-center z-0">
            <video
              ...
              muted
              playsInline
              className="w-[2000px] h-[2000px] object-contain opacity-15"
            />
          </div>
        )}
      ...
      <div className="fixed right-8 top-1/2 transform -translate-y-1/2 z-20 space-y-4">
        {[...Array(4)].map((_, i) => (
          <div
            key={i}
            className={`w-2 h-2 rounded-full cursor-pointer transition-all ${i === 0 ? 'bg-orange-500 scale-150' : 'bg-white/30 hover:bg-white/60'
              }`}
          />
        ))}
      </div>
